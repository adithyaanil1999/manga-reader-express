...
app.post("/autocomplete", (req, res) => {
  if (req.body.type === "manga") {
    try {
      http.get(
        `https://myanimelist.net/search/prefix.json?type=manga&keyword=${req.body.title}&v=11`,
        function (response) {
          var chunks = [];
        ...
        .then(function (response) {
          ...
          $("a").each((i, el) => {
            if (i < 10) {
              ...
              res.send({ comicSuggest: comicArr });
...
app.post("/search", (req, res) => {
  ...
  if (req.body.type === "manga") {
            ...
            }
            {
              ...
              http.get(url, (resp) => {
                ...
                resp.on("end", () => {
                  ...
                  {
                    let url =
                      "https://mangapark.net/search?orderby=views_a&q=" +
                      encodeURI(title);
                    http.get(url, (resp) => {
                      let html = "";
    ...
    try {
      ...
      data.append("keyword", req.body.title);
      var config = {
        method: "post",
            ...
            .each((i, el) => {
              chapterList.push({
                chapterTitle: $(el)
                  ...
                  .text(),
                chapterLink:
                  "https://www.mangahere.cc" + $(el).children("a").attr("href"),
                chapDate: $(el)
                  .children("a")
                  .children(".detail-main-list-main")
                  .children(".title2")
      ...
      .then(async function (response) {
        ...
        function parseChapterList(list) {
          ...
          for (let index = 0; index < list.length; index++) {
            i = list[index];
            currentTitle =
              "V." + (i.volume === "" ? 0 : i.volume) + " " + "Ch." + i.chapter;
